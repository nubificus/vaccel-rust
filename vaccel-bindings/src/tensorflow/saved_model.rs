...
impl SavedModel {
    ...
    fn set_path(&mut self, path: PathBuf) -> Result<()> {
        ...
        match unsafe { ffi::vaccel_tf_saved_model_set_path(self.inner, c_path.as_ptr()) as u32 } {
            ffi::VACCEL_OK => Ok(()),
            err => Err(Error::Runtime(err)),
        }
    }
    ...
    pub fn from_export_dir(&mut self, path: PathBuf) -> Result<()> {
        self.set_path(path)?;
    ...
    pub(crate) fn inner(&self) -> &ffi::vaccel_tf_saved_model {
        &(*self.inner)
    }
