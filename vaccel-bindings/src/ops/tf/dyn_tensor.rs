...
impl DynTensor {
    ...
    pub fn from_data_unchecked(dims: &[i64], data_type: DataType, data: Vec<u8>) -> Result<Self> {
        ...
        match unsafe {
            ffi::vaccel_tf_tensor_set_data(ptr, data.as_mut_ptr() as *mut _, data.len()) as u32
        } {
            ffi::VACCEL_OK => (),
            err => return Err(Error::Ffi(err)),
        }
    ...
    pub(crate) fn into_raw_parts(
        mut self,
    ) -> (NonNull<ffi::vaccel_tf_tensor>, bool, Option<Vec<u8>>) {
...
impl_component_drop!(DynTensor, vaccel_tf_tensor_delete, inner, owned);
