...
impl Buffer {
    ...
    pub fn new(data: Vec<u8>) -> Result<Self> {
        let mut buffer = data;
        let mut ptr: *mut ffi::vaccel_tf_buffer = ptr::null_mut();
        match unsafe {
            ffi::vaccel_tf_buffer_new(&mut ptr, buffer.as_mut_ptr() as *mut _, buffer.len()) as u32
        } {
            ffi::VACCEL_OK => (),
...
impl Drop for Buffer {
    fn drop(&mut self) {
        if self.owned {
            ...
            unsafe { ffi::vaccel_tf_buffer_take_data(self.inner.as_ptr(), &mut data, &mut size) };

            let ret = unsafe { ffi::vaccel_tf_buffer_delete(self.inner.as_ptr()) } as u32;
