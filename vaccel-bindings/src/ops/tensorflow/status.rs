...
impl Status {
    ...
    pub fn new(error_code: u8, message: &str) -> Result<Self> {
        ...
        match unsafe { ffi::vaccel_tf_status_new(&mut ptr, error_code, c_message.as_ptr()) as u32 }
        {
            ffi::VACCEL_OK => (),
    ...
    pub fn code(&self) -> u8 {
        unsafe { self.inner.as_ref().error_code }
    }
    ...
    pub unsafe fn populate_ptr(&self, ptr: *mut ffi::vaccel_tf_status) -> Result<()> {
        ...
        ffi.error_code = self.inner.as_ref().error_code;
        unsafe {
