...
impl DynTensor {
    ...
    pub fn new(dims: &[i64], data_type: DataType) -> Result<Self> {
    ...
    pub fn new_unchecked(dims: &[i64], data_type: DataType, data_size: usize) -> Result<Self> {
    ...
    pub fn from_data(dims: &[i64], data_type: DataType, data: Vec<u8>) -> Result<Self> {
        let data_count = Self::calculate_data_count(dims)?;
        let data_type_size = data_type.try_size_of().ok_or(Error::InvalidArgument(
            "No Rust type mapping for the provided `DataType`".to_string(),
        ))?;
    ...
    ///
    /// The caller must ensure the data size is correct for the tensor.
    pub fn from_data_unchecked(dims: &[i64], data_type: DataType, data: Vec<u8>) -> Result<Self> {
        let mut data = data;
        let mut ptr: *mut ffi::vaccel_torch_tensor = ptr::null_mut();
        match unsafe {
        ...
        match unsafe {
            ffi::vaccel_torch_tensor_set_data(ptr, data.as_mut_ptr() as *mut _, data.len()) as u32
        } {
            ffi::VACCEL_OK => (),
            err => return Err(Error::Ffi(err)),
        }
    ...
    fn verify_data_count<T: TensorType>(&self) -> Result<usize> {
        ...
        if expected_data_count != data_count {
            return Err(Error::ConversionFailed(format!(
                "Unexpeted data count; expected {} got {}",
    ...
    pub(crate) fn into_raw_parts(
        mut self,
    ) -> (NonNull<ffi::vaccel_torch_tensor>, bool, Option<Vec<u8>>) {
...
impl_component_drop!(DynTensor, vaccel_torch_tensor_delete, inner, owned);
...
impl From<&DynTensor> for TorchTensor {
...
impl From<DynTensor> for TorchTensor {
    fn from(dyn_tensor: DynTensor) -> Self {
        TorchTensor::from(&dyn_tensor)
    }
}

impl TryFrom<&TorchTensor> for DynTensor {
    type Error = Error;
